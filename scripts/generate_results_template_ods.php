...
function contentXml(array $cities, array $catalogue, bool $includeAthlete): string {
    $groups=[]; $tests=[]; $eventRows=odsRow(['Ámbito','Grupo','Prueba','Característica técnica'],'Header'); $count=0;
    foreach($catalogue as $area=>$items){foreach($items as $group=>$events){if(!in_array($group,$groups,true))$groups[]=$group;foreach($events as $event){if(!in_array($event,$tests,true))$tests[]=$event;$eventRows.=odsRow([$area,$group,$event,in_array($group,['Tanques','Llançaments'],true)?'Sí':'No']);$count++;}}}
    $headers=$includeAthlete?['Atleta',...HEADERS]:HEADERS;
    $blank=$includeAthlete?odsCell():'';
    $blank.=odsCell('','','Ambitos').odsCell('','','Grupos').odsCell('','','Pruebas').odsCell().odsCell().odsCell().odsCell('','','Ciudades').odsCell();
    ...
    $instructions=odsRow(['Plantilla con desplegables estándar'],'Header').odsRow(['Rellena únicamente la hoja Resultados.']).odsRow(['Las hojas Pruebas y Ciudades permiten consultar y filtrar valores.']).odsRow(['Guarda la hoja Resultados como CSV antes de importarla.']);
    $valid='<table:content-validations>'
        .'<table:content-validation table:name="Ambitos" table:condition="'.conditionValues(array_keys($catalogue)).'" table:allow-empty-cell="true" table:display-list="unsorted"/>'
        .'<table:content-validation table:name="Grupos" table:condition="'.conditionValues($groups).'" table:allow-empty-cell="true" table:display-list="unsorted"/>'
        .'<table:content-validation table:name="Pruebas" table:condition="'.conditionValues($tests).'" table:allow-empty-cell="true" table:display-list="unsorted"/>'
    ...
    return '<?xml version="1.0" encoding="UTF-8"?>'
        ...
        .'<office:automatic-styles><style:style style:name="Header" style:family="table-cell"><style:text-properties fo:font-weight="bold"/><style:table-cell-properties fo:background-color="#306334"/></style:style></office:automatic-styles><office:body><office:spreadsheet>'.$valid
        .'<table:table table:name="Resultados"><table:table-column table:number-columns-repeated="'.($includeAthlete?9:8).'"/>'.$results.'</table:table>'
        .'<table:table table:name="Pruebas"><table:table-column table:number-columns-repeated="4"/>'.$eventRows.'</table:table>'
        ...
        .'<table:database-ranges><table:database-range table:name="FiltroPruebas" table:target-range-address="$Pruebas.$A$1:$D$'.($count+1).'" table:display-filter-buttons="true"/><table:database-range table:name="FiltroCiudades" table:target-range-address="$Ciudades.$A$1:$A$'.(count($cities)+1).'" table:display-filter-buttons="true"/></table:database-ranges>'
