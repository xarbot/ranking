...
function importMarks(PDO $db,array $payload): array{$rows=$payload['marks']??null;if(!is_array($rows))throw new ApiException('No se han recibido marcas.');$athleteId=requiredId($payload,'athleteId','atleta');ensureExists($db,'atletas',$athleteId);$errors=[];$valid=[];foreach($rows as $index=>$row){try{if(!is_array($row))throw new ApiException('Fila no valida.');$row['athleteId']=$athleteId;$valid[]=$row;markPayload($db,$row);}catch(ApiException $e){$errors[]=['row'=>$index+2,'error'=>$e->getMessage()];}}if($errors)return ['imported'=>0,'errors'=>$errors];foreach($valid as $row)writeMark($db,$row);return ['imported'=>count($valid),'errors'=>[]];}
function importMultipleMarks(PDO $db,array $payload): array{$rows=$payload['marks']??null;if(!is_array($rows))throw new ApiException('No se han recibido marcas.');$errors=[];$valid=[];foreach($rows as $index=>$row){try{if(!is_array($row))throw new ApiException('Fila no valida.');markPayload($db,$row);$valid[]=$row;}catch(ApiException $e){$errors[]=['row'=>(int)($row['_line']??$index+2),'error'=>$e->getMessage()];}}if($errors)return ['imported'=>0,'errors'=>$errors];foreach($valid as $row)writeMark($db,$row);return ['imported'=>count($valid),'errors'=>[]];}
...
function route(PDO $db,string $method,string $path,array $payload): array {
 ...
 if($method==='POST'&&$path==='/athletes/move-marks')return [moveAthleteMarks($db,$payload),200];
 ...
 if($method==='POST'&&$path==='/marks/import/multiple')return [importMultipleMarks($db,$payload),200];
 if($method==='POST'&&$path==='/marks/replace')return [replaceMarkFields($db,$payload),200];
 ...
 if(preg_match('#^/(users|events|athletes|marks|translations)/(\d+)$#',$path,$m)){$resource=$m[1];$id=(int)$m[2];if($method==='PUT'){if($resource==='users')updateUser($db,$id,$payload,$actor);elseif($resource==='events')writeEvent($db,$payload,$id);elseif($resource==='athletes')writeAthlete($db,$payload,$id);elseif($resource==='marks')writeMark($db,$payload,$id);else writeTranslation($db,$payload,$id);return [['ok'=>true],200];}if($method==='DELETE'){if($resource==='users'){deleteUser($db,$id,$actor);}else{$table=['events'=>'pruebas','athletes'=>'atletas','marks'=>'marcas','translations'=>'traducciones'][$resource];ensureExists($db,$table,$id);execute($db,"DELETE FROM {$table} WHERE id=?",[$id]);}return [['ok'=>true],200];}}
